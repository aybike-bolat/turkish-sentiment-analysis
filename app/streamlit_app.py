...
def predict(text: str, vectorizer, model):
    ...
    if hasattr(model, "predict_proba"):
        try:
            ...
            # Eğer çıktı bir matris veya array ise ilk satırını alıyoruz
            if hasattr(proba_values, "toarray"):
                proba_values = proba_values.toarray()
            
            # Versiyon hatası (AttributeError) dahil her türlü kütüphane hatasını yakalıyoruz
            proba = dict(zip(model.classes_, proba_values[0]))
        except Exception:
            # Kütüphane uyumsuzluğu varsa uygulamayı çökertme, olasılığı pas geç
            proba = None
...
def render_comparison_tab():
    ...
    if comparison_path.exists():
        ...
        st.warning("Henüz model karşılaştırma sonucu bulunamadı. Önce `src/train_models.py` çalıştırılmalı.")
